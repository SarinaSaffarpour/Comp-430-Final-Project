...
os.makedirs("exp2_upperbound_plots", exist_ok=True)
...
mnist_test.targets += 10
train_set = ConcatDataset([cifar_train, mnist_train])
test_set = ConcatDataset([cifar_test, mnist_test])
...
torch.save(model.state_dict(), "resnet18_exp2_upperbound.pth")
print("Model saved to resnet18_exp2_upperbound.pth")

plt.figure()
plt.plot(range(1, num_epochs + 1), train_losses, marker='o')
plt.title("Training Loss (CIFAR+MNIST)")
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.grid()
plt.savefig("exp2_upperbound_plots/train_loss.png")
plt.close()
...
plt.title("Accuracy (CIFAR+MNIST)")
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.legend()
plt.grid()
plt.savefig("exp2_upperbound_plots/accuracy.png")
plt.close()

print("Plots saved to exp2_upperbound_plots/")