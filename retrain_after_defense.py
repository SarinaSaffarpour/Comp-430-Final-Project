...
EPOCHS = 20
BATCH_SIZE = 64
SAVE_DIR = "spectral_defense_retrain_plots"
os.makedirs(SAVE_DIR, exist_ok=True)

poison_mask = np.load("spectral_defense_output/poison_mask.npy")
clean_indices = np.where(~poison_mask)[0]
...
for epoch in range(EPOCHS):
    ...
    print(f"Epoch {epoch+1:02d} | Loss: {avg_loss:.4f} | Train Acc: {acc:.3f} | Test Acc: {test_acc:.3f}")

torch.save(model.state_dict(), "resnet18_retrained_after_defense.pth")

plt.figure()
plt.plot(range(1, EPOCHS + 1), train_losses, marker='o')
plt.title("Training Loss (Cleaned Dataset)")
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.grid()
...
plt.plot(range(1, EPOCHS + 1), test_accs, label="Test", marker='x')
plt.title("Accuracy (After Spectral Defense)")
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.legend()
plt.grid()
